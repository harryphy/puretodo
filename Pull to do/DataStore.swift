...
class DataStore {
    ...
    private let widgetTodoItemsKey = "widgetTodoItems"
    private let widgetTodoCountKey = "widgetTodoCount"
    ...
    func saveItems(_ items: [TodoItem], _ doneItems: [TodoItem], _ pinnedItems: [TodoItem]) {
        ...
        sharedDefaults.synchronize()
        Task { @MainActor in
            TodoImageStore.shared.purgeExpiredOriginals(doneItems: doneItems)
        }
        updateWidgetSnapshot(items: items, pinnedItems: pinnedItems, categories: loadCategories())
    ...
    func saveCategories(_ categories: [Category]) {
        ...
        keyValueStore.synchronize()
        sharedDefaults.synchronize()
        updateWidgetSnapshot(items: loadItems(), pinnedItems: loadPinnedItems(), categories: categories)
        WidgetCenter.shared.reloadTimelines(ofKind: "TodoWidget")
    }
    ...
    private func loadDataWithFallback<T: Encodable>(forKey key: String, decodeFunction: (String) -> T?) -> T? {
        // 先尝试从 iCloud 加载
        ...
        if let iCloud = iCloudData as? [TodoItem], let local = localData as? [TodoItem] {
            ...
            saveToSharedDefaults(iCloud, forKey: key)
            return iCloudData
        ...
        // 如果只有一种数据源，返回它
        if let iCloudData {
            saveToSharedDefaults(iCloudData, forKey: key)
        }
        return iCloudData ?? localData
    }

    func refreshWidgetSnapshot() {
        updateWidgetSnapshot(items: loadItems(), pinnedItems: loadPinnedItems(), categories: loadCategories())
    }

    private func updateWidgetSnapshot(items: [TodoItem], pinnedItems: [TodoItem], categories: [Category]) {
        let widgetItems = todoItemsForWidget(items: items, pinnedItems: pinnedItems, categories: categories)
        if let encodedWidgetItems = try? JSONEncoder().encode(widgetItems) {
            sharedDefaults.set(encodedWidgetItems, forKey: widgetTodoItemsKey)
            sharedDefaults.set(widgetItems.count, forKey: widgetTodoCountKey)
            sharedDefaults.synchronize()
        }
    }

    private func todoItemsForWidget(items: [TodoItem], pinnedItems: [TodoItem], categories: [Category]) -> [TodoItem] {
        let allItems = pinnedItems + items
        guard let toDoCategoryId = categories.first(where: { $0.name == "To Do" })?.id else {
            return allItems.filter { $0.categoryId == nil }
        }
        return allItems.filter { $0.categoryId == toDoCategoryId || $0.categoryId == nil }
    }

    private func saveToSharedDefaults<T: Encodable>(_ value: T, forKey key: String) {
